...
public class ScrollingViewHeaderLayout extends FrameLayout implements NestedScrollingParent {
    ...
    /**
     * 这里如果用{@link OverScroller 在有些手机上} {@link OverScroller#getCurrVelocity()} 为0
     */
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            isSeriesScroll = a.getBoolean(R.styleable.ScrollingViewHeaderLayout_isSeriesScroll, true);
    ...
    protected void onFinishInflate() {
        ...
        if (childCount == 2 && scrollingLayout == null) {
            scrollingLayout = findViewById(R.id.scroll_layout);

            scrollingScrollView =  findViewById(R.id.scroll_view);
            scrollingRecyclerView = findViewById(R.id.recycler_view);

            scrollingScrollView.post(() -> {
                measuredWidth = scrollingScrollView.getMeasuredWidth();
            });
        }

        if (headerView == null || scrollingRecyclerView == null) {
            throw new RuntimeException("please set header layout and scrolling layout");
        }
    }

    private View getTouchView(int X){
        // 测量第一个Viw
        if (X < measuredWidth){
            return scrollingScrollView;
        }else return scrollingRecyclerView;
    }

    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        if (scrollingLayout != null && headVisibleMinHeight > 0) {
            scrollingLayout.measure(widthMeasureSpec,
                    MeasureSpec.makeMeasureSpec(
                            scrollingLayout.getMeasuredHeight() - headVisibleMinHeight,
                            MeasureSpec.EXACTLY));
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        scrollingLayout.layout(left, getPaddingTop() + headerViewHeight, right,
                getPaddingTop() + headerViewHeight + scrollingLayout.getMeasuredHeight());
    }
    ...
    private
    @Nullable
    View getCurrentScrollView(int X) {
        View mView = getTouchView(X);
        return findScrollingView(mView);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                onMoveDown(ev);
                currScrollingView = getCurrentScrollView((int) ev.getX(0));
    ...
    public boolean scrollingViewIsTop() {
        ...
        return !currScrollingView.canScrollVertically(-1);
    }
    ...
    private int translateChild(int dy) {
        ...
        scrollingLayout.setTranslationY(scrollDistance);
